...
import { ArrowLeft, Save, Plus, Trash2, ArrowUp, ArrowDown, Download, Eye, EyeOff, Sparkles } from "lucide-react";
...
export default function EditCustomItineraryPage() {
  ...
  function addItineraryDay() { setItinerary([...itinerary, { day: itinerary.length + 1, title: "", description: "", activities: [], recommendations: [], meals: [], accommodation: "", images: [] }]); }
                ...
                <div className="space-y-4">
                  {flights.map((f, i) => (
                    <div key={f._key} className={`border rounded-xl p-4 space-y-3 ${f.type === "train" ? "border-orange-200 bg-orange-50/30" : "border-slate-200"}`}>
                      <div className="flex items-center justify-between">
                        <span className={`text-sm font-bold ${f.type === "train" ? "text-orange-600" : "text-cyan-700"}`}>{f.type === "train" ? "🚆" : "✈"} {f.type === "train" ? "Train" : "Flight"} {i + 1}</span>
                        <div className="flex items-center gap-2">
                  ...
                  {flights.length === 0 && <p className="text-sm text-slate-400 text-center py-4">No flights or trains added.</p>}
                </div>
              </>
            )}
          </>
